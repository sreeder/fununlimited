...
if (strlen($upc))
{
	...
	$sql = "SELECT pla_platformID,pla_name,itm_itemID,itm_title FROM items,platforms WHERE itm_upc='".mysql_escape_string($upc)."' AND itm_platformID=pla_platformID";
	$result = mysql_query($sql,$db);
	if (mysql_errno()) { ?><script type="text/javascript">alert('MySQL error in begqty UPC check - tell Scott!\n\n<?=mysql_escape_string($sql);?>\n<?=mysql_escape_string(mysql_error());?>')</script><?php }
	...
	if (mysql_num_rows($result))
	{
		...
		<script type="text/javascript">
			...
			if ($row['pla_platformID'] == $platformID)
			{
				...
				?>
				alert('UPC number <?=$upc;?> exists in the current platform, but is on a different page.\n\nThe <?=($newused==ITEM_NEW?'new':'used');?> quantity has been increased to <?=$updqty;?> for item: <?=mysql_escape_string($title);?>');
				parent.lastval = '';
				<?php
			}
			else { ?>parent.upc_exists('<?=$upc;?>','<?=mysql_escape_string($title);?>','<?=mysql_escape_string($name);?>');<?php }
