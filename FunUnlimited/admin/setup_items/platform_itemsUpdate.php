...
$return = @$_POST['return'];
if (!strlen($return)) { $return = '/admin/setup_items/platform_items_form.php'; }

$pg = new admin_page();
$error = new error('Whole Platform Item Info - Update');
	...
	if ($doupdate && is_array(@$_POST['info']))
	{
		...
		while (list($itemID,$arr) = each($info))
		{
			...
			while (list($prefix,$values) = each($by_prefix))
			{
				...
				{
					$vals = array();
					while (list($k,$v) = each($values)) { $vals[] = "$k='".mysql_real_escape_string(stripslashes($v))."'"; }
					$sqls[] = "UPDATE items SET ".implode(',',$vals)." WHERE itm_itemID=$itemID";
				}
				elseif ($prefix == "qty")
				{
					$vals = array();
					while (list($k,$v) = each($values)) { $vals[] = "$k='".mysql_real_escape_string($v)."'"; }
