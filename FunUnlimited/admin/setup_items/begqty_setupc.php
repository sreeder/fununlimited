...
if (strlen($idx) && strlen($itemID))
{
	if (strlen($upc))
	{
		...
		$sql = "SELECT pla_name,itm_title FROM items,platforms WHERE itm_upc='".mysql_real_escape_string($upc)."' AND itm_platformID=pla_platformID AND itm_active=".YES;
	...
	if (strlen($upc) && mysql_num_rows($result))
	{
		...
		<script type="text/javascript">
			parent.upc_exists('<?=$upc;?>','<?=mysql_real_escape_string($title);?>','<?=mysql_real_escape_string($name);?>');
			parent.unlock_upc();
		</script>
		<?php
	}
